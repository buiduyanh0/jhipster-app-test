...
type RestOf<T extends IBorrow | NewBorrow> = Omit<T, 'borrowDate' | 'returnDate'> & {
...
export class BorrowService {
  ...
  protected convertDateFromClient<T extends IBorrow | NewBorrow | PartialUpdateBorrow>(borrow: T): RestOf<T> {
    return {
      ...borrow,
      borrowDate: borrow.borrowDate?.toJSON() ?? null,
      returnDate: borrow.returnDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restBorrow: RestBorrow): IBorrow {
    return {
      ...restBorrow,
      borrowDate: restBorrow.borrowDate ? dayjs(restBorrow.borrowDate) : undefined,
      returnDate: restBorrow.returnDate ? dayjs(restBorrow.returnDate) : undefined,
    };
  }
