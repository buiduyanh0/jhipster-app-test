 ...
 */
public class MemberGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all members").get("/api/members").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new member")
                        .post("/api/members")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created member").get("${new_member_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created member").delete("${new_member_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Member entity").exec(scn);
